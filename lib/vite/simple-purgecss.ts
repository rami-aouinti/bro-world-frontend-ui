  ...
  return {
      ...
      await Promise.all(
        files.map(async file => {
          try {
            ...
            this.warn(`simple-purgecss: unable to read ${path.relative(rootDir, file)} - ${(error as Error).message}`)
          }
        }),
      )
    ...
    generateBundle(_, bundle) {
      for (const chunk of Object.values(bundle)) {
        ...
        const rawSource = typeof chunk.source === 'string' ? chunk.source : Buffer.from(chunk.source).toString('utf8')
...
function purgeCss(css: string, usedClasses: Set<string>, selectorSafelist: RegExp[]): string {
  ...
  root.walkRules(rule => {
    ...
    const selectors = (rule.selectors ?? rule.selector.split(',')).map(selector => selector.trim()).filter(Boolean)
    const keptSelectors = selectors.filter(selector => shouldKeepSelector(selector, usedClasses, selectorSafelist))
...
function shouldKeepSelector(selector: string, usedClasses: Set<string>, selectorSafelist: RegExp[]): boolean {
