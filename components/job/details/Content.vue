<template>
  <div class="mt-6">
    ...
    <div v-if="requirementList.length" class="mt-6">
      <h4 class="text-subtitle-1 font-weight-semibold mb-3">
        {{ t("job.requirements") }}
      </h4>
      <ul class="pl-5 text-body-2 text-medium-emphasis">
        <li v-for="requirement in requirementList" :key="requirement">
          {{ requirement }}
        </li>
      </ul>
    </div>

    <div v-if="benefitList.length" class="mt-6">
      <h4 class="text-subtitle-1 font-weight-semibold mb-3">
        {{ t("job.benefits") }}
      </h4>
      <ul class="pl-5 text-body-2 text-medium-emphasis">
        <li v-for="benefit in benefitList" :key="benefit">
          {{ benefit }}
        </li>
      </ul>
    </div>
