<template>
  ...
  >
    <template #image>
      <v-img cover :gradient="barGradient"></v-img>
    </template>
    <AppBrand class="ml-2" />
    <AppNavButtons
    ...
    <div v-if="showInlineSearch" class="hidden flex-1 items-center px-4 md:flex">
      <LayoutSearchButton class="w-full max-w-2xl" />
    </div>
    <div v-else-if="showSearchButton" class="hidden md:flex">
      <LayoutSearchButton />
    </div>
...
const appBarColor = computed(() => props.isDark ? 'rgba(12, 10, 22, 0.78)' : 'rgba(255, 255, 255, 0.82)')
const showInlineSearch = computed(() => !config.value.search.inAside && config.value.search.style === 'input')
const showSearchButton = computed(() => !config.value.search.inAside && config.value.search.style === 'button')
...
function changeLocale(value: string) { emit('update:locale', value) }

async function handleUserMenuSelect(item: UserMenuItem) {
  if (item.action === 'logout') {
    if (loggingOut.value) return
    loggingOut.value = true
    try { await auth.logout() } finally { loggingOut.value = false }
    return
  }
