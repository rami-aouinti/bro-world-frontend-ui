...
.app-loading-fade-leave-active { transition: opacity .35s ease; }
...
.app-loading-overlay {
  ...
  background:
      ...
      radial-gradient(circle at 80% 10%, rgba(34,211,238,.12), transparent 60%),
      radial-gradient(circle at 50% 100%, rgba(99,102,241,.18), transparent 65%),
      hsl(var(--background) / .93);
...
.app-loading-navbar {
  ...
  background: linear-gradient(150deg, rgba(15,23,42,.9), rgba(30,41,59,.82));
  backdrop-filter: blur(10px) saturate(120%);
  border-bottom: 1px solid rgb(3,32,61); z-index: 1;
}
.app-loading-navbar-inner {
  width: 100%; display: grid; grid-template-columns: auto 1fr auto;
  align-items: center; gap: clamp(.75rem, 2vw, 1rem);
}
.app-loading-brand { width: 11rem; height: 1.75rem; border-radius: .6rem; }
.app-loading-search { height: 2.2rem; border-radius: 999px; width: min(100%, 560px); justify-self: center; }
.app-loading-actions { display: flex; align-items: center; gap: .5rem; }
.app-loading-action { width: 2rem; height: 2rem; border-radius: .75rem; }
@media (max-width: 720px) {
  .app-loading-navbar-inner { grid-template-columns: auto auto; }
  .app-loading-search { display: none; }
}
...
.app-loading-column { display: flex; flex-direction: column; gap: 1.25rem; }

/* --- Sidebar gauche --- */
.app-loading-aside-card { border-radius: 1.25rem; height: auto; }
.app-loading-aside-card--nav { padding: .8rem 0; }
.app-loading-nav { display: grid; gap: .85rem; padding: .35rem .5rem; }
.app-loading-nav-item { height: 2.8rem; border-radius: 999px; }
...
.app-loading-card {
  ...
  border-radius: 1rem; padding: clamp(1rem, 2.2vw, 1.25rem);
  background: linear-gradient(150deg, rgba(15,23,42,.9), rgba(30,41,59,.82));
  border: 1px solid rgba(148,163,184,.14);
  box-shadow:
      0 18px 36px rgba(15,23,42,.45),
      0 8px 16px rgba(15,23,42,.28);
}
.app-loading-card-header { display: flex; align-items: center; gap: .65rem; }
.app-loading-avatar { width: 2.25rem; height: 2.25rem; border-radius: 999px; }
.app-loading-meta { flex: 1; display: flex; flex-direction: column; gap: .5rem; }
.app-loading-body { display: grid; gap: .55rem; }
.app-loading-line { height: .85rem; border-radius: 999px; }
.app-loading-line--medium { width: 80%; }
.app-loading-line--short { width: 55%; }
.app-loading-footer { display: flex; gap: .65rem; flex-wrap: wrap; }
.app-loading-tag { height: 1.5rem; width: clamp(4.2rem, 25%, 6rem); border-radius: 999px; }
...
.app-loading-aside-card--tall-2 { height: clamp(7rem, 18vw, 7rem); }
...
.app-loading-skeleton::after {
  ...
  background: linear-gradient(
      120deg,
      transparent 15%,
      rgba(248,250,252,.18) 45%,
      rgba(248,250,252,.28) 55%,
      transparent 85%
  );
...
@media (max-width: 1024px) {
  .app-loading-scene { grid-template-columns: repeat(2, minmax(0, 1fr)); }
  .app-loading-column--aside {
    order: 3; display: grid; grid-template-columns: repeat(2, minmax(0, 1fr)); gap: 1rem;
  }
  .app-loading-grid { grid-template-columns: repeat(2, minmax(0, 1fr)); }
}
@media (max-width: 720px) {
  .app-loading-overlay { padding: 1.25rem; }
  .app-loading-scene { grid-template-columns: 1fr; }
  .app-loading-column--feed { order: 1; }
  .app-loading-column--nav { order: 2; }
  .app-loading-column--aside { order: 3; grid-template-columns: 1fr; }
  .app-loading-grid { grid-template-columns: 1fr; }
}
