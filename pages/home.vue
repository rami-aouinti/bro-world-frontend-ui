...
const profileStore: ReturnType<typeof useProfileStore> | null = (() => {
  try {
    ...
    if (import.meta.dev) {
      console.warn("[pages/home] Profile store unavailable; using fallback profile state.", error);
    }
...
function resolveNuxtApp(): NuxtAppLike {
  try {
    ...
    if (import.meta.dev) {
      console.warn("[pages/home] Falling back to a mock Nuxt app instance.", error);
    }
  }
...
function resolveSharedState<T>(key: string, initializer: () => T) {
  if (typeof useState === "function") {
    try {
      ...
      if (import.meta.dev) {
        console.warn(`[pages/home] Falling back to local state for "${key}".`, error);
      }
    }
  }
...
try {
  ...
  if (import.meta.dev) {
    console.warn("[pages/home] Unable to import useSeoMeta helper.", error);
  }
}
...
const initialAuthAccessState = resolveSharedState(
  "home:initial-can-access-authenticated-content",
...
watch(
  ...
  (postItems) => {
    ...
    void callOnceFn("home:prefetch-first-blog-post-card", prefetchBlogPostCard);
  },
  { immediate: true, flush: "post" },
);

const initialSkeletonVisibilityState = resolveSharedState(
  "home:initial-show-authenticated-skeletons",
...
const initialLoadError = resolveSharedState<string | null>("home:initial-load-error", () => null);
...
await callOnceFn("pages:home:initial-posts", async () => {
...
<style scoped src="~/assets/styles/pages/home.scss" lang="scss"></style>
