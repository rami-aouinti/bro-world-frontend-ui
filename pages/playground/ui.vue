<template>
  <v-container fluid class="ui-playground py-10">
    ...
    <v-card class="ui-playground__toolbar" rounded="lg" elevation="2">
      <div class="ui-playground__toolbar-grid">
        <div class="ui-playground__toolbar-item">
          <span class="ui-playground__toolbar-label">{{ t('pages.playground.ui.toggles.locale') }}</span>
          <v-btn-toggle v-model="selectedLocale" mandatory divided>
            <v-btn v-for="option in localeOptions" :key="option.value" :value="option.value">
              {{ option.label }}
            </v-btn>
          </v-btn-toggle>
        </div>
        <div class="ui-playground__toolbar-item">
          <span class="ui-playground__toolbar-label">{{ t('pages.playground.ui.toggles.direction') }}</span>
          <v-btn-toggle v-model="direction" mandatory divided>
            <v-btn value="ltr">{{ t('pages.playground.ui.directions.ltr') }}</v-btn>
            <v-btn value="rtl">{{ t('pages.playground.ui.directions.rtl') }}</v-btn>
          </v-btn-toggle>
        </div>
        <div class="ui-playground__toolbar-item">
          <span class="ui-playground__toolbar-label">{{ t('pages.playground.ui.toggles.theme') }}</span>
          <v-btn-toggle v-model="themeMode" mandatory divided>
            <v-btn value="light">{{ t('pages.playground.ui.themes.light') }}</v-btn>
            <v-btn value="dark">{{ t('pages.playground.ui.themes.dark') }}</v-btn>
          </v-btn-toggle>
        </div>
        <div class="ui-playground__toolbar-item">
          <span class="ui-playground__toolbar-label">{{ t('pages.playground.ui.toggles.density') }}</span>
          <v-btn-toggle v-model="activeSize" mandatory divided>
            <v-btn v-for="sizeOption in sizeOptions" :key="sizeOption" :value="sizeOption">
    ...
    <v-expansion-panels v-model="expandedPanels" multiple class="ui-playground__panels">
      ...
      >
        ...
        <v-expansion-panel-text>
          <div v-for="section in group.sections" :key="section.id" class="ui-playground__section">
            <div class="ui-playground__section-header">
              <h3 class="text-subtitle-1 font-weight-semibold mb-1">{{ section.label }}</h3>
              <p v-if="section.description" class="text-body-2 text-medium-emphasis mb-0">
                {{ section.description }}
              </p>
            </div>
            <div class="ui-playground__examples">
              <article v-for="example in section.examples" :key="example.id" class="ui-playground__example">
                <div class="ui-playground__example-header">
                  ...
                  <p v-if="example.notes" class="ui-playground__example-notes text-caption">
                    {{ example.notes }}
                  </p>
                </div>
...
const selectedLocale = ref(localeOptions.value.some((option) => option.value === locale.value) ? locale.value : 'en')
...
.ui-playground__example-code {
  font-family: 'JetBrains Mono', ui-monospace, SFMono-Regular, SFMono-Regular, Menlo, Monaco, Consolas, 'Liberation Mono', 'Courier New', monospace;
  font-size: 0.75rem;
