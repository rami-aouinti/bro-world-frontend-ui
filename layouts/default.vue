<template>
  <v-app :theme="isDark ? 'dark' : 'light'" :style="cssVars">
          ...
          <ClientOnly>
            ...
            <template #fallback>
              <div class="pane-scroll px-3 py-4">
                <div class="flex flex-col gap-4">
                  <v-skeleton-loader type="list-item-two-line" class="rounded-2xl" />
                  <v-skeleton-loader v-for="index in 2" :key="index" type="card" class="rounded-2xl" />
                </div>
              </div>
            </template>
          </ClientOnly>
    ...
    <v-main v-if="areSidebarsVisible" class="app-surface">
